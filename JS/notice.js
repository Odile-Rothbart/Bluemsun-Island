...
function request(pager){
    ...
    $.ajax({
        type: 'GET',
        url:"http://windlinxy.top:8080/bluemsun_island/master_sections?cur="+Number(pager.currentPage)+"&size="+Number(pager.limit),
        headers:{
            "Authorization":token
        },
            ...
            if(data.status==1){
                ...
                for(var item=0;item< data.page.list.length;item++){
                    if(data.page.list[item].sectionStatus!=-1){
                        dataHtml += `<div class="data" onclick="jumpsection(${data.page.list[item].sectionId})">
                                        <div class="user">
